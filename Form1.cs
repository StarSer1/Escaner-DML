...
namespace Escaner_DML
{
    public partial class Form1 : Form
    {
        ...
        List<string> tokensForm1 = new List<string>();
        ...
        public Form1()
        {
            ...
            tokens2 = Analisis.Analizador2(txtEntrada, DgvLexica, txtError, DtTablas, DtAtributos, DtRestricciones);
            Analisis.LLENADOTABLASPAPU(DtTablas, DtAtributos, DtRestricciones, tokensForm1);
            txtEntrada.Text = "";
        }
        ...
        private void BtnAnalizar_Click(object sender, EventArgs e)
        {
            if (txtEntrada.Text != "")
            {
                ...
                {
                    tokens2 = Analisis.Analizador2(txtEntrada, DgvLexica, txtError, DtTablas, DtAtributos, DtRestricciones);
                    tokensForm1.Add(tokens2[0]);
                    // Ahora las copias temporales no se verán afectadas por los cambios en las originales

                    if (Analisis.errorActivado == false)
                    {
                        Analisis.LLENADOTABLASPAPU(DtTablas, DtAtributos, DtRestricciones, tokens2);
                        Analisis.LlenadoSelects(tokens2);
                        try
                        {
                            while (tokens2.Last() == "\n")
                            {
                            ...
                            errorActivado = Analisis.Sintaxis(tokensForm1, tokens2, txtError, txtEntrada, DgvLexica, txtError, DtTablas, DtAtributos, DtRestricciones);
