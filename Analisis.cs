...
//             4     8             10           11    12    13    14    15    50    51    52    53    54    61    62    72    99
        ...
        public string Analizador(RichTextBox texto, DataGridView dgvLex, TextBox txtError, DataGridView dgvTabla, DataGridView dgvAtributos, DataGridView dgvRestriccion)
        {
            ...
            for (int i = 0; i < texto.TextLength; i++)
            {
                ...
                if (!delimitadores.IsMatch(c))
                {
                    ...
                    {
                        if (dondevoy + 1 < texto.TextLength)
                        {
                            char siguienteChar = texto.Text[dondevoy + 1];
                    ...
                    if (c == " " || c == "\n")
                    {
                        if (palabraConComilla != true)
                        {
                            return actual = cadena;
                        }
                        if(palabraConComilla == true && c == " ")
                        {
                            cadena += c;
                        }
                ...
                else
                {
                    if ("'’‘".Contains(c))
                    {
                        ...
                        palabraConComilla = !palabraConComilla;
                    }
                    if (c == "(")
                        acumuladorParentesisAbierto++;
                    else if (c == ")")
        ...
        public void cadText(List<string> tokens)
        {
                                ...
                                if (X == ConvertirToken(K))
                                {
                                    ...
                                    if (apun < rep)
                                        if (primeravez != true)
                                        {
                                            ...
                                            if((insertPendiente == true || truncatePendiente == true) && actual == ")")
                                            {
                                                int pasos = 0;
                                                ...
                                                if (truncatePendiente == true)
                                                {
                                                    dos(apun - pasos, tokens, texto, lineas, error);
                                                    truncatePendiente = false;
                                                }
                                            }
                                            if (valorAntesN == true)
                                            {
